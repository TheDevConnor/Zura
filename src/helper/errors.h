...
inline void error_parser(Token line_no, int pos, const char *msg) {
  cout << "\n[" << termcolor::yellow << "line: " << termcolor::cyan
       << line_no.line << termcolor::reset << "] "
       << "[" << termcolor::yellow << "pos: " << termcolor::cyan << pos - 1
       << termcolor::reset << "] Error: ";
  ...
  if (line_no.kind == EOF_TOKEN)
    ...
    cout << "at " << termcolor::red << "'" << termcolor::reset
         << termcolor::cyan << line_no.length << termcolor::reset
  ...
  // Print the line
  cout << termcolor::magenta << string(line_start - 1, line_end - line_start)
       << termcolor::reset << "\n";
...
inline void runtimeError(const char *format, ...) {
  ...
  if (vm.frames->closure->function->name != nullptr) {
    cout << "[" << termcolor::yellow << "line" << termcolor::reset << " -> "
         << termcolor::red << line << termcolor::reset << "]["
         << termcolor::yellow << "pos" << termcolor::reset << " -> "
         << termcolor::red << instruction << termcolor::reset << "]["
         << termcolor::red << "func" << termcolor::reset << " -> "
