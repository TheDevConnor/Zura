...
inline void run_file(const char* path)
{
    ...
    if (result == Zura::Exit_Value::COMPILATION_ERROR)
        Zura::Exit(Zura::Exit_Value::COMPILATION_ERROR);
    if (result == Zura::Exit_Value::RUNTIME_ERROR)
        Zura::Exit(Zura::Exit_Value::RUNTIME_ERROR);
}
