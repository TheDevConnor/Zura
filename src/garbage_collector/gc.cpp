...
void collect_garbage() {
    ...
    cout << "collected " << before - vm.bytes_allocated << " bytes (from " << before
         << " to " << vm.bytes_allocated << ") next at " << vm.next_gc;
    #endif
}
...
// Function to recursively mark the value of an upvalue and any other object it references
    ...
    if (IS_OBJ(value)) {
        ...
        if (!object->is_marked) {
            ...
            switch (object->type) {
                ...
                case OBJ_UPVALUE: {
                    recursive_mark(((ObjUpvalue*)object)->closed); // Recursive call to mark the value
                    break;
                }
                default:
                    break;
